...
export function RegisterPage(props: RegisterPageProps) {
  ...
  const schema = yup.object().shape({
    ...
    username: yup
      ...
      .test(
        "Mã khach hàng có đủ 5 chữ số",
        "Mã khách hàng phải bắt đầu bằng A và có 5 chữ số đằng sau",
        (values) => {
          return values.length === 6 && values[0] === "A"
        },
      )
      .matches(/^A\d{5}$/, "Mã khách hàng không hợp lệ"),
    phoneNumber: yup
      .string()
  ...
  useEffect(() => {
    if (actionAuth == "Failed") {
      enqueueSnackbar("Mã khách hàng đã được sử dụng", {
        variant: "error",
      })
    }
  }, [actionAuth])
          ...
          {msv?.length === 0 && !next ? (
            <div className="flex items-center flex-col pt-[15vh]">
              ...
              <p className="text-gray-500 text-center text-sm mb-10">
                ...
                có phải là sinh viên hay không. Do đây là chương
              ...
              <p className="text-gray-500 text-sm text-center mt-5">
                Ảnh phải rõ ràng không mờ, và đầy đủ mã khách hàng, mã vạch,..
                <a
                ...
                . Nếu trường hợp bị sai mã khách hàng vui lòng F5 để thực hiện
                lại
              </p>
            </div>
          ) : (
